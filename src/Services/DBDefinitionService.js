...
class DBDefinitionService {
      ...
      try {
        SQLite.echoTest().then(() => {
          ...
          SQLite.openDatabase(DB_CONFIG.dbName, DB_CONFIG.dbVersion, DB_CONFIG.dbDisplayname, DB_CONFIG.dbSize).then((DB) => {
            ...
            this.populateDatabase(DB, isDbVersionChanged).then(res => {
              resolve(true);
            }).catch(err => {
              reject(false);
            });
    ...
    return new Promise((resolve, reject) => {
      ...
      db.executeSql('SELECT 1 FROM ' + TABLES.TBL_CALL_ADDED + ' LIMIT 1').then(() => {
        ...
        if (isDbVersionChanged === true) {
          this.checkAndAddNewColumn().then((res) => {
            resolve(true);
          }).catch((error) => {
            resolve(true);
          });
  ...
  async checkAndAddNewColumn() {
    ...
    return await new Promise((resolve, reject) => {
      DB_CONFIG.tables.forEach((table) => {
        let colCheckQuery = "SELECT sql FROM sqlite_master WHERE tbl_name = '" + table.name + "' AND type = 'table'";
        this.query(colCheckQuery).then((result) => {
          ...
          let promiseQueryCheck = new Promise((resolve, reject) => {
            ...
            let commaStr = qryStr.substring(firstIndex + 1, qryStr.length - 1);
            ...
            if (isFound2 === undefined) {
              ...
              let createNewquery = 'CREATE TABLE IF NOT EXISTS ' + table.name + '_NEW (' + columns.join(',') + ');';
              //console.log('db -> createNewquery:: ', createNewquery);
              this.query(createNewquery).then(res => {
                //console.log('Success: ', res);
                // STEP - 2 -> Copy data
                let insertQuery = "INSERT INTO " + table.name + "_NEW SELECT " + columnNames.join(',') + " FROM " + table.name;
                //console.log('db -> insertQuery:: ', insertQuery);
                this.query(insertQuery).then(res => {
                  //console.log('Success: ', res);
                  // STEP - 3 -> Drop old table
                  let dropQuery = "DROP TABLE IF EXISTS " + table.name;
                  //console.log('db -> createNewquery:: ', dropQuery);
                  this.query(dropQuery).then(res => {
                    //console.log('Success: ', res);
                    // STEP - 4 -> Rename new into old
                    let renameQuery = "ALTER TABLE " + table.name + "_NEW RENAME TO " + table.name;
                    //console.log('db -> createNewquery:: ', renameQuery);
                    this.query(renameQuery).then(res => {
                      //console.log('Success: ', res);
                    }).catch((err) => {
          ...
          promiseQueryCheck.then(res => resolve(true)).catch(err => reject(err));
