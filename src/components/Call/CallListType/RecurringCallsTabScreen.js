...
export default class RecurringCallsTabScreen extends Component {
  ...
  _renderItem_callColorsRow = ({ item, index }) => (
    <CallDateRow
      ...
      index={index}
      item={item}
      dataArr={this.props.screenProps.recurringArr.filter(row => moment(row.generatedScheduleDate).format("YYYY-MM-DD") === item)}
  ...
  render() {
      return (
          ...
          >
            <FlatList
              data={this.props.screenProps.recurringDatesArr}
