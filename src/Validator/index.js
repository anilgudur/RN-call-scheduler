...
export default class ValidationComponent extends Component {
  ...
  */
  ...
  // Concatenate each error messages | Added by Anil G on 29/06/2017
  getErrorMessagesArray(separator="\n") {
    return this.errors;
  }

  // Concatenate each error messages
  getErrorMessage(fieldName, separator="\n") {
    let [error] = this.errors.filter(err => err.fieldName === fieldName);
    return error;

    // return error.map(function(elem){
    //     return elem.messages;
    // }).join(separator);

    //return this.errors[fieldName].map((messages) => messages.join(separator)).join(separator);
    //return (this.errors.filter(err => err.fieldName === fieldName).then((err) => err.messages.join(separator)).join(separator));
    //return this.errors.map((err) => err.messages.join(separator)).join(separator);
  }
}
